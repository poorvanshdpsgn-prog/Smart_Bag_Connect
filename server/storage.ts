...
export interface IStorage {
  ...
  updateTimetable(day: string, meeting: string, meetingTime: string): Promise<Timetable>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateTimetable(day: string, meeting: string, meetingTime: string): Promise<Timetable> {
    const existing = await db.select().from(timetables).where(eq(timetables.day, day));

    if (existing.length > 0) {
      const [updated] = await db
        .update(timetables)
        .set({ meeting, meetingTime, updatedAt: new Date() })
      ...
      const [created] = await db
        .insert(timetables)
        .values({ day, meeting, meetingTime })
        .returning();
      return created;
    }
  }
